...
import ssl # og running
# import OpenSSL.SSL as ssl #pm checking
import paho.mqtt.client as mqtt
# import urllib3
# # Configure urllib3 to use pyOpenSSL
# urllib3.contrib.pyopenssl.inject_into_urllib3()

from AnalyticsJsonHandler import AnalyticsJsonHandler  # Make sure to import this correctly
import threading
...
class AnalyticsMqtt:
    ...
    def __init__(self, filename, subscribe_callback, connect_success_cb=None, service_name=None):
        try:
            ...
            self.microservice_name = self.load_mqtt_settings['microservice_name']
            # print(self.microservice_name,self.service_name,"-------------purbasha prints-------------------")
            # Velo_fft_microservice
            # AnalyticsMQTT - ------------purbasha
            # prints - ------------------

            self.broker_cert_loc = "C:\\VELO_FFT_service\\cert"#"C:\\VELO_FFT_service\\cert"  # "cert"
    ...
    def readfile(self):
        try:
            self.json_handler_object = AnalyticsJsonHandler(self.filename)
    ...
    def configure_ssl(self):
        try:
            if self.load_mqtt_settings[self.broker_key]['ssl_auth_enabled'] == 1:
                ca_cert = self.broker_cert_loc + "/" + self.load_mqtt_settings[self.broker_key]['ssl_auth_details'][
                    'ca_certificate']
                # print("ca_cert path:", ca_cert)

                client_cert = self.broker_cert_loc + "/" + self.load_mqtt_settings[self.broker_key]['ssl_auth_details'][
                    'client_certificate']
                client_key = self.broker_cert_loc + "/" + self.load_mqtt_settings[self.broker_key]['ssl_auth_details'][
                    'client_key_file']
                ...
                #                     keyfile_password=self.load_mqtt_settings[self.broker_key]['ssl_auth_details'][
                #                         'keyfile_password'])
    ...
    def connect_broker(self):
        try:
            ...
            if (self.load_mqtt_settings[self.broker_key]['broker_username'] != "None" and
                    self.load_mqtt_settings[self.broker_key]['broker_password'] != "None"):
                self.client.username_pw_set(
                    username=self.load_mqtt_settings[self.broker_key]['broker_username'],
                    password=self.load_mqtt_settings[self.broker_key]['broker_password']
                )

            if (self.load_mqtt_settings[self.broker_key]['broker_ip'] != "None" and
                    self.load_mqtt_settings[self.broker_key]['broker_port'] != "None"):
                try:
                    self.client.connect(self.load_mqtt_settings[self.broker_key]['broker_ip'],
                                        self.load_mqtt_settings[self.broker_key]['broker_port'],keepalive=25)  # keepalive
        ...
        try:
            ...
            if result_code == 0:
                ...
                payload_msg = "%s: MQTT broker Connected Successful to %s at %s" % (
                    self.microservice_name, self.load_mqtt_settings[self.broker_key]['broker_ip'], time.time())
                logging.info(payload_msg)
        ...
        try:
            ...
            if result_code != 0:
                ...
                payload_msg = "%s: MQTT broker %s was disconnected at %s" % (
                    self.service_name, self.load_mqtt_settings[self.broker_key]['broker_ip'], time.time())
                logging.info(payload_msg)
                logging.info("rc is" + str(result_code))
    ...
    def add_subscribe_data(self, topic, message_callback): #topic [ +/MA/+]
        try:
            ...
            try:
                ...
                if self.client.connected_flag is False:
                    ...
                    logging.warning("Reconnecting to MQTT broker %s",
                                    self.load_mqtt_settings[self.broker_key]['broker_ip'])
                    time.sleep(0.5)
    ...
    #                 logging.error(payload_msg)
    #                 self.reconnect_broker()
    #         elif self.load_mqtt_settings[self.broker_key]['ssl_auth_enabled'] == 0:
