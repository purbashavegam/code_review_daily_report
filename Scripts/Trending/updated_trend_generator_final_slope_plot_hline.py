...
def moving_avg_trendline_threshold(original_df, dataframe, 
                             dataframe_name, df_name, config, directory_axis, 
                             data_duration_days):
    ...
    for method in trending_methods:
        ...
        if method['enabled'] and method['name'] == 'Moving Average and Trendline':
            ...
            # plt.title(f'{dataframe_name} Moving Average and Trendline (Window_size_ 1_Day) - {data_duration_days} days')
            # plt.title(f'{dataframe_name}_')
            plt.legend(loc='upper right')
            title_name = dataframe_name[:-5]+f'_{axis_name_for_report(df_name)}'
            # Save the trendline chart as an image in the specified output format
            trendline_output_file = os.path.join(directory_axis, f'5.{title_name}.{config["output_format"]}')
            plt.savefig(trendline_output_file, format=config['output_format'])
            # Close the plot to release resources (optional)
            plt.close()
...
def plotting(dataframes, output_folder):  # to plots and get pics
    ...
    for file_name, item_path, axis_name, dataframe in dataframes:
        ...
        dataframe = dataframe[(dataframe['DateTime'] <= '20/10/2023 11:59 PM')] 
        df = dataframe
        start_date = pd.to_datetime(df['DateTime'].min())
        end_date = pd.to_datetime(df['DateTime'].max())
        data_duration_days = ((end_date - start_date).days) + 1
        
        original_df = dataframe[(dataframe['DateTime'] <= '20/10/2023 11:59 PM')]       
        ...
        #                               data_duration_days)
        # original_df = 
        moving_avg_trendline_threshold(original_df, dataframe, 
                                     file_name, axis_name, config, directory_axis, 
                                     data_duration_days)
...
if __name__ == '__main__':
    ...
    for root, _, files in os.walk(root_directory):
        ...
        # print(data.columns)
        # print(dataframes)
        # end_date = '20/10/2023 11:59 PM'
        # print(data['DateTime'])
        # dataframes = data[(data['DateTime'] <= end_date)]
        # print(dataframes)
